...
func init() {
	...
	regexMap["token"] = fmt.Sprintf(`(?:%s|\-|\.|!|%%|\*|_|\+|`+"`"+`|'|~)+`,
		regexMap["ALPHANUM"]) // check this regex
	...
	regexMap["mark"] = `[-_\.!~\*'\(\)]`
	regexMap["alphanum"] = fmt.Sprintf(
		`^(%s|%s)$`, regexMap["ALPHA"], regexMap["DIGIT"],
	)
	...
	regexMap["iotl-char"] = fmt.Sprintf(`(?:%s|\-)`,
		regexMap["ALPHANUM"])
