...
func processPDU(payload []byte) (*SipMessage, []byte, error) {
	...
	for i := lnIdx; i < uint16(len(msglines)) && msglines[i] != ""; i++ {
		...
		if matches != nil {
			...
			case Max_Forwards.LowerCaseString():
				...
				if err != nil {
					LogError(LTSIPStack, fmt.Sprintf("Invalid Max-Forwards header - %v", err.Error()))
				} else if max < 0 || max > 255 {
					LogError(LTSIPStack, "Invalid Max-Forwards header - Too little/big")
				} else {
					sipmsg.MaxFwds = max
				}
			...
			case CSeq.LowerCaseString():
				...
				if startLine.StatusCode == 0 {
					...
					if r1 != r2 {
						LogError(LTSIPStack, fmt.Sprintf("Invalid Request Method: %v vs CSeq Method: %v", r1, r2))
						return nil, nil, errors.New("invalid CSeq header")
					}
				}
	...
	if !strings.Contains(cntntType, "multipart") {
		...
		if bt == Unknown {
			LogError(LTBadSIPMessage, "Unknown Content-Type header")
		} else {
		...
		for {
			...
			case Unknown:
				LogError(LTBadSIPMessage, "Unknown Content-Type header - skipped")
			default:
	...
	if sipmsg.IsRequest() {
		...
		case ACK:
			if trans.Method == INVITE {
				...
				go ss.mediaReceiver()
				go ss.startRTPStreaming("RaafatAlHagganIntro")
			} else { //ReINVITE
